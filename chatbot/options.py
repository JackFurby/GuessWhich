...
def readCommandLine(argv=None):
    ...
    #-------------------------------------------------------------------------
    # Data input settings
    parser.add_argument('-inputImg', default=os.path.join(settings.BASE_DIR, 'chatbot/data/data_img.h5'),
                            help='HDF5 file with image features')
    parser.add_argument('-inputQues', default=os.path.join(settings.BASE_DIR, 'chatbot/data/chat_processed_data.h5'),
                            help='HDF5 file with preprocessed questions')
    parser.add_argument('-inputJson', default=os.path.join(settings.BASE_DIR, 'chatbot/data/chat_processed_params.json'),
                            help='JSON file with info and vocab')
    parser.add_argument('-inputDenseJson', default='chatbot/data/visdial_1.0_val_dense_annotations.json',
    ...
    # get full file path for model paths
    if parsed['startFrom']:
        parsed['startFrom'] = os.path.join(settings.BASE_DIR, parsed['startFrom'])
    if parsed['qstartFrom']:
        parsed['qstartFrom'] = os.path.join(settings.BASE_DIR, parsed['qstartFrom'])
    return parsed
